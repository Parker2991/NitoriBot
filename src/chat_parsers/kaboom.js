...
function kaboom(message, data) {
  ...
  if (isSeparatorAt(children, 1)) {
    // Missing/blank display name
  ...
  if (data.uuid) {
    ...
    sender = data.players.find((player) =>
      util.isDeepStrictEqual(player.displayName, playerListDisplayName),
    );
  }
...
function isSeparatorAt(children, start) {
  return (
    (children[start]?.text === ":" || children[start]?.text === "\xa7f:") &&
    children[start + 1]?.text === " "
  );
}
