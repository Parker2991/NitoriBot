const BOOLEAN_TYPES = ['interpret', 'bold', 'italic', 'underlined', 'strikethrough', 'obfuscated']
function convertNbtComponentToJson (key, tag) {
    ...
    if (!tag) {
      ...
      for (const key of Object.keys(tag.value)) {
        object[key === '' ? 'text' : key] = convertNbtComponentToJson(key, tag.value[key])
      }
      ...
      for (const item of tag.value.value) {
        array.push(convertNbtComponentToJson(null, { type: tag.value.type, value: item }))
      }
