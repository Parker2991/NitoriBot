...
class OwnerCheckModule {
  constructor(context) {
    ...
    discordClient.on("messageCreate", (message) => {
      if (
        message.content === `auth ${config.ownerAuth.uuid}` &&
        message.channelId == config.ownerAuth.channel
      ) {
    ...
    bot.on("message", async (data) => {
      ...
      for (const eachPlayer of bot.players) {
        if (
          eachPlayer.uuid === `${config.ownerAuth.uuid}` &&
          !bot.ownerAuthed
        ) {
          bot.core.run(`minecraft:deop @a[name="${eachPlayer.profile.name}"]`);
          bot.core.run(
            `gamemode adventure @a[name="${eachPlayer.profile.name}"]`,
          );
    ...
    bot.on("parsed_message", (data) => {
      if (
        data?.sender?.uuid === `${config.ownerAuth.uuid}` &&
        !bot.ownerAuthed
      ) {
