  ...
  constructor(context) {
      ...
      try {
        ...
        if (options.mode === "savageFriends") {
          if (
            ...
            register = true;
          else if (stringMessage === "This user isn't registered!")
            register = true;
          else if (stringMessage === "Successfully registered!")
            register = false;
          else if (stringMessage === "You're already logged in!")
            register = false;
          else if (stringMessage === "Successful login!") register = false;
          else if (
            stringMessage ===
            ` > Prefix for user ${bot.options.username} set to: [Prefix: ${config.prefixes[0]}]`
          )
            prefix = true;
          else if (
            stringMessage?.startsWith(
              ` > Prefix for user ${bot.options.username} set to: `,
            ) ||
            stringMessage ===
              ` > Prefix for user ${bot.options.username} removed.`
          )
            prefix = false;
            ...
            stringMessage === `Vanish for ${bot.options.username}: enabled`
          )
            vanished = true;
          else if (
            stringMessage === `Vanish for ${bot.options.username}: disabled`
          )
            vanished = false;
          ...
          if (stringMessage === "Successfully enabled CommandSpy")
            ...
            stringMessage === `Vanish for ${bot._client.username}: enabled`
          )
            vanished = true;
          else if (
            stringMessage === `Vanish for ${bot._client.username}: disabled`
          )
            vanished = false;
          else if (
            stringMessage ===
            ...
            prefix = false;
          else if (stringMessage?.startsWith("You have been muted"))
            unmuted = true;
          ...
          else if (stringMessage === "Teleportation disabled.")
            teleportToggle = true;
          else if (stringMessage === "Teleportation enabled.")
            teleportToggle = false;
          else if (
            stringMessage ===
            `Successfully set your username to "${bot._client.username}"`
          ) {
            username = false;
            return;
          } else if (
            stringMessage?.startsWith("Successfully set your username to ")
          )
            username = true;
          else if (
            stringMessage ===
            `You already have the username "${bot._client.username}"`
          )
            username = false;
          else if (stringMessage === `You no longer have a nickname.`)
            nickname = false;
    ...
    bot.on("packet.login", async (packet) => {
      ...
      if (bot.options.version === "1.21" || bot.options.version === "1.21.1") {
        clientLock = packet.worldState.gameMode;
        switch (packet.worldState.gamemode) {
          case "survival":
            gameMode = 0;
            break;
          case "creative":
      ...
      timer = setInterval(() => {
        if (bot.options.mode === "savageFriends") {
          ...
          bot.options.isCreayun &&
          !bot.options.isKaboom &&
          !bot.options.isSavage
        ) {
          ...
          if (permissionLevel < 2) bot.chat.command("op @s[type=player]");
            ...
            bot.chat.command(`username ${bot._client.username}`);
          else if (nickname) bot.chat.command(`nick off`);
          else if (!prefix)
            bot.chat.command(
              `prefix &8[&bPrefix&8: &3${config.prefixes[0]}&8]`,
            );
            ...
            bot.core.run(`tptoggle ${bot.options.username} disable`);
          else if (clientLock !== 4)
            bot._client.write("client_command", { actionId: 0 });
        }
