  ...
  constructor (context) {
    ...
    bot.on('system_chat', (message) => {
      try {
        ...
        const sender = bot.players.find((e) => e?.profile?.name === message.text)?.profile?.name === message.text
        ...
        if (
          ...
          displayName: bot.players.find((e) => e.profile.name === message.text).displayName
        })
      } catch (e) {
        console.log(e.stack)
      }
